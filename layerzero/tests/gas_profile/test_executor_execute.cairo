...
use crate::constants::SENT_MESSAGE_SIZE_LIMIT;
use crate::e2e::utils::{
    deploy_dvn, deploy_endpoint, deploy_executor, deploy_price_feed, deploy_treasury,
    deploy_ultra_light_node_302, wire_oapp, wire_ultra_light_node_302,
};
use crate::gas_profile::utils::{
    DVN_ADMIN, DVN_CALL_DATA_EXPIRATION, DVN_DST_CONFIG, DVN_OWNER, ENDPOINT_OWNER, EXECUTOR_ADMIN,
    EXECUTOR_DST_CONFIG, EXECUTOR_ROLE_ADMIN, LOCAL_EID, LOCAL_OAPP_OWNER, MAX_MESSAGE_SIZE,
    MESSAGE_LIB_OWNER, PRICE_FEED_OWNER, REMOTE_EID, REMOTE_OAPP, TREASURY_OWNER, build_byte_array,
    build_dvn_signers, build_dvn_verification_call_data, build_incoming_packet, create_uln_config,
    deploy_oapp, get_native_token,
};
...
fn execute(step: usize) {
    ...
    let packet = build_incoming_packet(
        oapp.address, build_byte_array(SENT_MESSAGE_SIZE_LIMIT / 2 * step / TOTAL_STEPS),
    );
    ...
    // We check a packet delivery event explicitly because the `execute` call on the executor
            ...
            @array![
                (
                    ...
                    EndpointV2::Event::PacketDelivered(
                        PacketDelivered { origin, receiver: oapp.address },
                    ),
                ),
            ],
        );
